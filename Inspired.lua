...
do
  ...
  function Polygon2:__eq(spatialObject)
    return spatialObject:__type() == "Polygon2" 
  end
      ...
      local function IsOk(m,_)
        if m and m.valid and not m.dead then
          for k,v in pairs(_)do
            ...
            elseif (type(v)=="function" and not v(m)) or not m[k] or m[k]~=v then
              return false
            end
          end
          return true
...
function print(...)
  ...
  for i=1, len do
    ...
    elseif _type == "Object" then 
      t[i] = "Object: "..v.charName
    elseif _type == "Vector" then 
      t[i] = tostring(v)
    elseif _type == "number" then 
      t[i] = tostring(v)
    elseif _type == "table" then 
      t[i] = table.serialize(v)
    elseif _type == "boolean" then 
      t[i] = ({[true]="True",[false]="False"})[v]
...
function FileExist(path) -- Will Be Deleted
...
local hasDAC = FileExist(SCRIPT_PATH.."Deftsu's Auto Carry.lua") or FileExist(COMMON_PATH.."Deftsu's Auto Carry.lua")
local hasIOW = FileExist(SCRIPT_PATH.."IOW.lua") or FileExist(COMMON_PATH.."IOW.lua")
local hasPW = FileExist(SCRIPT_PATH.."Platywalk.lua") or FileExist(COMMON_PATH.."Platywalk.lua")

function LoadOrb(i)
  if i == 1 and hasIOW and not IOW then
    LoadIOW()
    _G.IOW_Loaded = true
  elseif i == 2 and hasDAC and not DAC_Loaded then
    require("Deftsu's Auto Carry")
    LoadDAC()
  elseif i == 3 and hasPW and not PW then
    require("Platywalk")
    LoadPW()
    _G.PW_Loaded = true
  end
...
mc_cfg_orb:DropDown("orb", "Select your orbwalker:", 1, {"IOW", "DAC"})
mc_cfg_orb:Boolean("on", "Enable", true, function() print("Please reload for this change to take effect. (2xF6)") end)
if mc_cfg_orb.on:Value() then
  LoadOrb(mc_cfg_orb.orb:Value())
end
